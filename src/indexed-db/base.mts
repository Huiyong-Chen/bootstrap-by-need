 ...
 */
export async function performDatabaseUpgrade(roleId: string, existingRoles: RoleInfoRecord[]) {
  // 如果已有升级在进行，等待其完成
  if (upgradeLock) {
    await upgradeLock;
 ...
 * @returns Promise
 */
export function promisifyRequest<T = unknown>(request: IDBRequest<T>): Promise<T> {
  return new Promise((resolve, reject) => {
