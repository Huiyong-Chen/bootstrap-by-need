import { QuestionBanks, QuestionByType, RoleInfo, RoleInfoRecord } from "../types/index.types.js";
...
// 初始化数据库
function _initDB(version?: number) {
...
function ensureRoleStoreExists(db: IDBDatabase, roleId: string) {
  const storeName = getRoleStoreName(roleId);
  return db.objectStoreNames.contains(storeName);
}

// 获取角色对应的 objectStore 名称
function getRoleStoreName(roleId: string) {
  return `role_${roleId}`;
}
// 升级数据库以创建新角色的 objectStore
async function upgradeDBForNewRole(roleId: string) {
  // 如果已有升级在进行，等待其完成
  if (upgradeLock) {
    await upgradeLock;
...
export async function queryAllRoles(): Promise<RoleInfoRecord[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      db.close();
      reject(
        new Error(`读取角色列表失败: ${request.error?.message || "未知错误"}`)
      );
    };
...
// 创建或更新角色信息
export async function saveRoleInfo(role: RoleInfo | RoleInfoRecord) {
  ...
  // 保存角色信息
  return new Promise<RoleInfoRecord>((resolve, reject) => {
    ...
    request.onerror = () => {
      db.close();
      reject(
        new Error(`保存角色信息失败: ${request.error?.message || "未知错误"}`)
      );
    };
  });
}
...
export async function getQuestionBankByRole(
  ...
  return new Promise<QuestionByType | null>((resolve, reject) => {
    const transaction = db.transaction([storeName], 'readonly')
    const store = transaction.objectStore(storeName)
    const request = store.get('data')

    request.onsuccess = () => {
      const internalBank: QuestionByType = request.result || {}
      db.close()
