...
import { generatePaper, loadQuestionBanks } from "../datas/questions.js";
...
import { QuestionTypeChineseName, QuestionTypeOrder } from "../types/index.types.js";
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function setRoleBadgeText(roleName) {
        DOMS.roleBadge.innerText = `当前岗位：${roleName}`;
    }
    // 当前角色题库
    let questionBanks = null;
    let questionRatioList = {};
    // 根据所选角色，渲染题库权重
    async function renderQuestionsRatioAndPreviewExport(roleId) {
    ...
    function renderQuestionRatio() {
        ...
        questionTypes.forEach((type) => {
            ...
            const typeRatioEle = document.createElement("input");
            ...
            typeRatioEle.onchange = e => {
                questionRatioChange(e.target.value, questionType);
            };
    ...
    function questionRatioChange(value, type) {
    ...
    function renderQuestionPage(questions, totalScore, remainingScore) {
    ...
    async function handleExport() {
        ...
        const timestamp = new Date()
            ...
            .replace(/[-:]/g, '')
            .slice(0, 15);
        const examDoc = buildDoc(`${label} 随机试卷（总分 ${generatedPaper.totalScore}）`, generatedPaper.list, false);
