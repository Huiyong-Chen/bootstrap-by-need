...
今天早上我只是把 `User` 对象的 `name` 字段拆分成了 `firstName` 和 `lastName`。
这是一个很常见的重构。
结果，我在浏览器里点点点，跑通了主流程，却在下午发现一个边缘的工具函数崩了——因为它还在尝试读取 `user.name.trim()`。
...
目前加载的 JS 文件只有 20 个，总耗时不到 100ms。
浏览器并没有因为请求过多而阻塞。
